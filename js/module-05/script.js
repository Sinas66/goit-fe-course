...
// //   return arr.reduce((acc,arr) => acc.concat(arr.prop), []);
// // };
...
//   return guests.map(el =>
//     el.inactiveDays > period ? (el.isActive = false) : (el.isActive = true)
//   );
// };
...
//     el.isActive ? acc.concat(el.name) : acc, []
//   );
//   // return guests.map(el => el.isActive )
// };
...
//   return guests.reduce((acc, el) => (el.age > age ? acc.concat(el) : acc), []);
// };
...
// const getGuestById = (guests, a) => {
//   // return guests.reduce((acc, el) => (el.id === a ? acc + el : acc), {});
//   return guests.find(el => (el.id === a));
// };
...
// const birthLate = mass => {
//   let result = 0;
//   mass.reduce((acc, el, ind) => {
//     if (mass[ind].year > result) {
//       result = mass[ind].year;
//     }
...
// console.log(birthLate(scientist));
...
const getUsersByEyeColor = (arr, color) => {
  ...
  return arr.filter(el => (el.eyeColor === color ? el : null));
};
...
const getUsersByGender = (arr, gender) => {
  return arr.reduce((acc, el) => {
    el.gender === gender ? acc.push(el.name) : null;
    return acc;
  }, []);
};
...
const getInactiveUsers = arr => {
  return arr.filter(el => (!el.isActive ? el : null));
};
...
const getUsersWithAge = (arr, min, max) => {
  return arr.reduce((acc, el) => {
    ...
    //   acc.push(el);
    // }
    age > min && age < max ? acc.push(el) : null;
    return acc;
  }, []);
};
...
const getUsersByFriend = (arr, name) => {
  return arr.reduce((acc, el) => {
    el.friends.includes(name) ? acc.push(el) : null;
    return acc;
  }, []);

  // return arr.friends.filter(el => el === name);
};
...
const getAllUniqueSkills = arr => {
  return arr.reduce((acc, el) => {
    el.skills.forEach(elm => (!acc.includes(elm) ? acc.push(elm) : null));
    return acc.sort();
  }, []);
};
...
const getUserNamesSortedByFriendsCount = arr => {
  let mass = arr.slice(0);
  mass.sort((a, b) => a.friends.length - b.friends.length);
  return mass.map(el => el.name);
};
